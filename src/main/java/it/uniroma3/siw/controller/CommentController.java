...
public class CommentController {
    ...
    public String addComment(@PathVariable Long productId,
        ...
        // salverebbe anche il nuovo commento (se la relazione è configurata con CascadeType.ALL o PERSIST)
        // productService.save(product, null); // L'immagine è null perché non la stiamo aggiornando
     ...
     * La sicurezza controlla che tu sia l'autore del commento o un admin.
     */
    ...
    /**
     * Cancella un commento in modo sicuro.
     * Sostituisce i tuoi due vecchi metodi di cancellazione.
     */
    @PostMapping("/comment/{id}/delete")
    @PreAuthorize("isAuthenticated() and (@commentService.findById(#id).get().author.credentials.email == authentication.principal.username or hasRole('ADMIN'))")
    public String deleteSecureComment(@PathVariable("id") Long id) {
        // Trova il prodotto a cui il commento appartiene PRIMA di cancellarlo, per il redirect
        Long productId = commentService.findById(id).get().getProduct().getId();
        commentService.deleteById(id);
        return "redirect:/product/" + productId;
    }
    
    /**
     * Cancella un commento dalla pagina del prodotto.
     */
    @PostMapping("/products/{productId}/comments/{id}/delete") // URL aggiornato
    public String deleteComment(@PathVariable Long productId,
                                @PathVariable Long id) {
        commentService.deleteById(id);
        return "redirect:/product/" + productId; // Redirect aggiornato
    }

    /**
     * Cancellazione da dashboard admin.
     */
    @PostMapping("/admin/comments/{id}/delete") // URL aggiornato
    @PreAuthorize("hasRole('ADMIN')")
    public String deleteCommentAdmin(@PathVariable Long id) {
        // Opzionale: potresti voler reindirizzare alla pagina del prodotto
        // Long productId = commentService.findProductId(id); 
        commentService.deleteById(id);
        return "redirect:/admin/dashboard"; // O un'altra pagina admin
    }
